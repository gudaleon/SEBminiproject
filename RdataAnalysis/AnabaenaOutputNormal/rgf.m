function [A, rec, Lux, ts, dvc, t, hcy, prhcy, fNlev, fClev, spltc, lngtt, M, P] = rgf(A, generations, Lux, ts)
...
fClev = zeros(1,generations+1);
Plev = zeros(1,generations+1);
fNlev(1) = sum([A.FixedNitrogen]);
lngtt(1) = length(A);
fClev(1) = sum([A.FixedCarbon]);
Plev(1) = sum([A.Phosphate]);
...
for p = 1:generations
    ...
    lngtt(p+1) = length(A);
    fNlev(p+1) = sum([A.FixedNitrogen]);
    fClev(p+1) = sum([A.FixedCarbon]);
    Plev(p+1) = sum([A.Phosphate]);
    rec = [rec;y(:,length(yend))];
    ts = dvc(p+1);
    M{p} = [A.DifferentiationFinished];
    P{p} = A;
    p
end
end
