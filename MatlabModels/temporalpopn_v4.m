...
dydt(2) = -p3*V1 + rd*V1*Z;                                              % V1
dydt(3) = V1*rp*circadian*ZL - rC*V1*Z - a*(1-circadian)*(C1/(C1+k)*V1); % C1
dydt(4) = 2*b*(Pext/(Pext + kPext))*V1 - rP*V1*Z;

% Species 2:
dydt(5) = 2*a*(C2/(C2 + k))*H2 - rN*V2*Z2;                               % N2
dydt(6) = -p3*V2 + rd*pv*V2*Z2;                                          % V2
dydt(7) = -p3*H2 + rd*ph*V2*Z2;                                          % H2
dydt(8) = ZL2*rp*V2 - rC*V2*Z2 - a*H2*C2/(C2 + k);                       % C2
